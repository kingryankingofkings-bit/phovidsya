...
impl ValidatedAdmission {
    pub const fn request_id(&self) -> u128 { self.request_id }
    pub const fn request_sequence(&self) -> u64 { self.request_sequence }
    pub const fn subject_id(&self) -> u128 { self.subject_id }
    pub const fn resource_id(&self) -> u128 { self.resource_id }
    pub const fn input_digest(&self) -> Digest { self.input_digest }
    pub const fn source(&self) -> DataTrust { self.source }
    pub const fn requested(&self) -> AuthorityClass { self.requested }
    pub const fn capability_id(&self) -> u128 { self.capability_id }
    pub const fn capability_issuer_id(&self) -> u128 { self.capability_issuer_id }
    pub const fn capability_authentication_evidence_digest(&self) -> Digest {
        self.capability_authentication_evidence_digest
    }
    pub const fn approval_id(&self) -> u128 { self.approval_id }
    ...
    pub const fn admitted_at_monotonic_ns(&self) -> u64 { self.admitted_at_monotonic_ns }
    pub const fn deadline_monotonic_ns(&self) -> u64 { self.deadline_monotonic_ns }
}
...
pub fn encode_capability_grant_preimage(
    ...
    if is_zero(&grant.input_digest) { return Err(MembraneError::ZeroDigest); }
...
pub fn encode_supervisor_approval_preimage(
    ...
    if is_zero(&approval.capability_authentication_evidence_digest) || is_zero(&approval.input_digest) {
...
fn validate_request(request: &MembraneRequest, context: &MembraneContext) -> Result<(), MembraneError> {
    ...
    if request.request_id == 0 || request.request_sequence == 0 || request.subject_id == 0
        || request.resource_id == 0 || request.capability_id == 0 || request.policy_generation == 0
        || context.current_policy_generation == 0 || context.trusted_capability_issuer_id == 0
    {
        return Err(MembraneError::MissingIdentity);
    }
    if is_zero(&request.input_digest) { return Err(MembraneError::ZeroDigest); }
...
fn validate_capability<V: BindingVerifier>(
    ...
    if grant.schema_major != request.schema_major || grant.schema_minor != request.schema_minor
        || grant.capability_id != request.capability_id || grant.request_id != request.request_id
        || grant.request_sequence != request.request_sequence || grant.subject_id != request.subject_id
        || grant.resource_id != request.resource_id || grant.source != request.source
        || grant.requested != request.requested || grant.input_digest != request.input_digest
    ...
    if !verifier.verify_capability(grant.issuer_id, &preimage, &grant.authentication_evidence_digest) {
        return Err(MembraneError::CapabilityAuthenticationFailed);
    }
    Ok(())
}
...
fn validate_approval<V: BindingVerifier>(
    ...
    if approval.supervisor_id == request.subject_id || approval.supervisor_id == capability.issuer_id {
        return Err(MembraneError::IndependentSupervisorRequired);
    }
    if is_zero(&approval.authentication_evidence_digest) {
        return Err(MembraneError::SupervisorApprovalAuthenticationFailed);
    }
    if approval.schema_major != request.schema_major || approval.schema_minor != request.schema_minor
        || approval.request_id != request.request_id
        || approval.request_sequence != request.request_sequence
        || approval.subject_id != request.subject_id || approval.resource_id != request.resource_id
        || approval.source != request.source || approval.requested != request.requested
        || approval.capability_id != request.capability_id
        || approval.capability_issuer_id != capability.issuer_id
        || approval.capability_authentication_evidence_digest != capability.authentication_evidence_digest
    ...
    if !verifier.verify_supervisor_approval(approval.supervisor_id, &preimage, &approval.authentication_evidence_digest) {
        return Err(MembraneError::SupervisorApprovalAuthenticationFailed);
    }
    Ok(())
}
...
mod tests {
    ...
    impl BindingVerifier for TestVerifier {
        fn verify_capability(&self, issuer_id: u128, canonical_preimage: &[u8], authentication_evidence_digest: &Digest) -> bool {
            *authentication_evidence_digest == test_evidence(0x43, issuer_id, canonical_preimage)
        }
        fn verify_supervisor_approval(&self, supervisor_id: u128, canonical_preimage: &[u8], authentication_evidence_digest: &Digest) -> bool {
    ...
    fn test_evidence(domain: u8, issuer: u128, bytes: &[u8]) -> Digest {
        ...
        for (index, byte) in issuer.to_be_bytes().iter().chain(bytes).enumerate() {
            ...
            out[slot] = out[slot].rotate_left(((index + slot) % 7 + 1) as u32).wrapping_add(*byte) ^ (index as u8).wrapping_mul(17);
        }
        out
    }
    ...
    fn request() -> MembraneRequest {
        MembraneRequest {
            ...
            request_id: 1, request_sequence: 11, subject_id: 2, resource_id: 9,
            input_digest: [1; 32], source: DataTrust::ModelGenerated,
            requested: AuthorityClass::Execute, capability_id: 3,
            policy_generation: 4, deadline_monotonic_ns: 20, server_claims_safe: true,
        }
    }
    ...
    fn context() -> MembraneContext {
        MembraneContext {
            schema_major: SCHEMA_MAJOR, schema_minor: SCHEMA_MINOR,
            now_monotonic_ns: 10, current_policy_generation: 4,
            trusted_capability_issuer_id: 100, trusted_supervisor_id: 300,
            last_accepted_request_sequence: 10,
            direct_privileged_path_absent: true, credentials_shared_with_server: false,
        }
    }

    fn unsigned_capability(request: &MembraneRequest) -> CapabilityGrant {
        CapabilityGrant {
            schema_major: request.schema_major, schema_minor: request.schema_minor,
            issuer_id: 100, capability_id: request.capability_id,
            request_id: request.request_id, request_sequence: request.request_sequence,
            subject_id: request.subject_id, resource_id: request.resource_id,
            source: request.source, requested: request.requested,
            input_digest: request.input_digest, policy_generation: request.policy_generation,
            issued_at_monotonic_ns: 5, not_before_monotonic_ns: 5,
    ...
    fn unsigned_approval(request: &MembraneRequest, capability: &CapabilityGrant) -> SupervisorApprovalReceipt {
        SupervisorApprovalReceipt {
            schema_major: request.schema_major, schema_minor: request.schema_minor,
            approval_id: 200, supervisor_id: 300,
            request_id: request.request_id, request_sequence: request.request_sequence,
            subject_id: request.subject_id, resource_id: request.resource_id,
            source: request.source, requested: request.requested,
            capability_id: request.capability_id, capability_issuer_id: capability.issuer_id,
            capability_authentication_evidence_digest: capability.authentication_evidence_digest,
            input_digest: request.input_digest, policy_generation: request.policy_generation,
            approved_at_monotonic_ns: 8, expires_at_monotonic_ns: request.deadline_monotonic_ns,
            request_deadline_monotonic_ns: request.deadline_monotonic_ns,
            authentication_evidence_digest: [0; 32],
        }
    }

    fn signed_approval(request: &MembraneRequest, capability: &CapabilityGrant) -> SupervisorApprovalReceipt {
        let mut approval = unsigned_approval(request, capability);
        let preimage = encode_supervisor_approval_preimage(&approval).unwrap();
        approval.authentication_evidence_digest = test_evidence(0x41, approval.supervisor_id, &preimage);
        approval
    }
    ...
    fn exact_authenticated_bindings_pass_and_are_preserved() {
        ...
        let validated = admit(&request, Some(&capability), Some(&approval), &mut context, &TestVerifier).unwrap();
    ...
    fn successful_admission_consumes_the_request_sequence() {
        let (request, capability, approval, mut context) = admitted();
        assert!(admit(&request, Some(&capability), Some(&approval), &mut context, &TestVerifier).is_ok());
    ...
    fn request_annotation_never_substitutes_for_capability_or_approval() {
        let request = request(); let mut context = context();
        assert!(request.server_claims_safe);
        assert_eq!(admit(&request, None, None, &mut context, &TestVerifier), Err(MembraneError::CapabilityMissing));
        let capability = signed_capability(&request);
        assert_eq!(
    ...
    fn capability_cannot_cross_subject_resource_authority_or_input() {
        let (request, capability, approval, mut context) = admitted();
        let mut changed = request; changed.resource_id += 1;
        assert_eq!(admit(&changed, Some(&capability), Some(&approval), &mut context, &TestVerifier), Err(MembraneError::CapabilityMismatch));
        changed = request; changed.subject_id += 1;
        assert_eq!(admit(&changed, Some(&capability), Some(&approval), &mut context, &TestVerifier), Err(MembraneError::CapabilityMismatch));
        changed = request; changed.requested = AuthorityClass::Device;
        assert_eq!(admit(&changed, Some(&capability), Some(&approval), &mut context, &TestVerifier), Err(MembraneError::CapabilityMismatch));
        changed = request; changed.input_digest = [0x77; 32];
        assert_eq!(admit(&changed, Some(&capability), Some(&approval), &mut context, &TestVerifier), Err(MembraneError::CapabilityMismatch));
    }
    ...
    fn cryptographically_valid_grant_from_an_unselected_issuer_rejects() {
        ...
        capability.authentication_evidence_digest = test_evidence(0x43, capability.issuer_id, &encode_capability_grant_preimage(&capability).unwrap());
        assert_eq!(admit(&request, Some(&capability), Some(&approval), &mut context, &TestVerifier), Err(MembraneError::CapabilityMismatch));
    }

    #[test]
    fn approval_cannot_be_replayed_across_request_or_target() {
        let (request, _, old_approval, mut context) = admitted();
        let mut changed = request; changed.request_id += 1; changed.request_sequence += 1;
        let capability = signed_capability(&changed);
        assert_eq!(admit(&changed, Some(&capability), Some(&old_approval), &mut context, &TestVerifier), Err(MembraneError::SupervisorApprovalMismatch));
        changed = request; changed.resource_id += 1;
        let capability = signed_capability(&changed);
        assert_eq!(admit(&changed, Some(&capability), Some(&old_approval), &mut context, &TestVerifier), Err(MembraneError::SupervisorApprovalMismatch));
    }
    ...
    fn self_future_expired_and_forged_approvals_reject() {
        let (request, capability, approval, mut context) = admitted();
        let mut self_approval = approval; self_approval.supervisor_id = request.subject_id;
        assert_eq!(admit(&request, Some(&capability), Some(&self_approval), &mut context, &TestVerifier), Err(MembraneError::IndependentSupervisorRequired));
        let mut future = unsigned_approval(&request, &capability);
        future.approved_at_monotonic_ns = context.now_monotonic_ns + 1;
        future.authentication_evidence_digest = test_evidence(0x41, future.supervisor_id, &encode_supervisor_approval_preimage(&future).unwrap());
        assert_eq!(admit(&request, Some(&capability), Some(&future), &mut context, &TestVerifier), Err(MembraneError::SupervisorApprovalFromFuture));
        let mut expired = unsigned_approval(&request, &capability);
        expired.expires_at_monotonic_ns = context.now_monotonic_ns - 1;
        expired.authentication_evidence_digest = test_evidence(0x41, expired.supervisor_id, &encode_supervisor_approval_preimage(&expired).unwrap());
        assert_eq!(admit(&request, Some(&capability), Some(&expired), &mut context, &TestVerifier), Err(MembraneError::SupervisorApprovalExpired));
        let mut forged = approval; forged.authentication_evidence_digest[0] ^= 1;
        assert_eq!(admit(&request, Some(&capability), Some(&forged), &mut context, &TestVerifier), Err(MembraneError::SupervisorApprovalAuthenticationFailed));
    }
    ...
    fn ambient_path_credentials_and_wall_time_are_never_trusted() {
        ...
        assert_eq!(admit(&request, Some(&capability), Some(&approval), &mut context, &TestVerifier), Err(MembraneError::AmbientBypass));
        context = self::context(); context.credentials_shared_with_server = true;
        assert_eq!(admit(&request, Some(&capability), Some(&approval), &mut context, &TestVerifier), Err(MembraneError::CredentialExposure));
