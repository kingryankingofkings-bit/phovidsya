...
impl ValidatedCommand {
    pub const fn command_id(&self) -> u128 { self.command_id }
    pub const fn command_sequence(&self) -> u64 { self.command_sequence }
    pub const fn device_id(&self) -> u128 { self.device_id }
    pub const fn iommu_domain_id(&self) -> u128 { self.iommu_domain_id }
    pub const fn queue_id(&self) -> u32 { self.queue_id }
    pub const fn region_id(&self) -> u128 { self.region_id }
    ...
    pub const fn validated_at_monotonic_ns(&self) -> u64 { self.validated_at_monotonic_ns }
    pub const fn uses_bounce_buffer(&self) -> bool { self.uses_bounce_buffer }
}
...
pub fn compile(
    ...
    if command.command_id == 0 || command.command_sequence == 0 || command.device_id == 0
        || command.iommu_domain_id == 0 || command.region_id == 0 || region.id == 0
        || region.device_id == 0 || region.iommu_domain_id == 0
        || context.trusted_device_id == 0 || context.trusted_iommu_domain_id == 0
        || context.authorized_region_id == 0 || command.generation == 0
        || region.generation == 0 || context.device_generation == 0
    {
    ...
    if command.region_id != region.id || command.device_id != region.device_id
        || command.iommu_domain_id != region.iommu_domain_id || command.queue_id != region.queue_id
    {
...
mod tests {
    ...
    fn region() -> RegionCapability {
        RegionCapability { id: 2, device_id: 11, iommu_domain_id: 12, queue_id: 7,
            size_bytes: 4096, generation: 13, allowed: Direction::ToDevice,
            mapped: true, quarantined: false }
    }
    fn command() -> DmaCommand {
        DmaCommand { schema_major: SCHEMA_MAJOR, schema_minor: SCHEMA_MINOR,
            command_id: 21, command_sequence: 31, device_id: 11, iommu_domain_id: 12,
            queue_id: 7, region_id: 2, offset: 0, length: 4096,
            direction: Direction::ToDevice, generation: 13, deadline_monotonic_ns: 40 }
    }
    fn context() -> CompileContext {
        CompileContext { schema_major: SCHEMA_MAJOR, schema_minor: SCHEMA_MINOR,
            now_monotonic_ns: 20, last_accepted_command_sequence: 30,
            trusted_device_id: 11, trusted_iommu_domain_id: 12, trusted_queue_id: 7,
            authorized_region_id: 2, device_generation: 13,
            iommu_isolated: true, bounce_pool_available: false, allow_zero_length: false }
    }
    #[test]
    ...
    #[test]
    fn replay_rejected() {
        let command = command(); let region = region(); let mut ctx = context();
        assert!(compile(&command, &region, &mut ctx).is_ok());
        assert_eq!(compile(&command, &region, &mut ctx), Err(DmaError::ReplayOrOutOfOrder));
    }
    #[test]
    fn region_target_mismatch_rejected() {
        let mut cmd = command(); cmd.device_id += 1;
        assert_eq!(compile(&cmd, &region(), &mut context()), Err(DmaError::RegionTargetMismatch));
    }
    #[test]
    fn trusted_target_mismatch_rejected() {
        let mut cmd = command(); let mut rgn = region();
        cmd.device_id += 1; rgn.device_id = cmd.device_id;
        assert_eq!(compile(&cmd, &rgn, &mut context()), Err(DmaError::TrustedTargetMismatch));
    }
    #[test]
    fn overflow_bounds_rejected() {
        let mut cmd = command(); cmd.offset = 4095; cmd.length = 2;
        assert_eq!(compile(&cmd, &region(), &mut context()), Err(DmaError::OutOfBounds));
    }
    #[test]
    fn stale_generation_rejected() {
        let mut ctx = context(); ctx.device_generation += 1;
        assert_eq!(compile(&command(), &region(), &mut ctx), Err(DmaError::StaleGeneration));
    }
    #[test]
    fn bounce_buffer_requires_pool() {
        let mut ctx = context(); ctx.iommu_isolated = false;
        assert_eq!(compile(&command(), &region(), &mut ctx), Err(DmaError::IsolationUnavailable));
        ctx.bounce_pool_available = true;
        assert!(compile(&command(), &region(), &mut ctx).unwrap().uses_bounce_buffer());
    }
}
