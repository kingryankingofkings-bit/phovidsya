...
pub fn admit(
    ...
    if policy.schema_major != 1 {
        return Err(MonitorError::UnsupportedSchema);
    }
    ...
    for digest in [
        ...
        request.validator_receipt_digest,
    ] {
        if digest.iter().all(|byte| *byte == 0) {
            return Err(MonitorError::ZeroDigest);
        }
    }
    if request.kernel_digest != policy.main_kernel_digest {
        return Err(MonitorError::UnrecognizedKernel);
    }
    if request.policy_epoch != policy.policy_epoch {
        return Err(MonitorError::StalePolicy);
    }
    if request.sequence == 0 || request.sequence <= context.last_sequence {
        return Err(MonitorError::Replay);
    }
    if request.deadline_monotonic_ns < context.now_monotonic_ns {
        return Err(MonitorError::Expired);
    }
    if !context.capability_bound_to_subject {
        return Err(MonitorError::SubjectCapabilityMismatch);
    }
    ...
    if matches!(
        request.resource,
        ResourceClass::Storage | ResourceClass::DmaDevice
    ) {
        if !context.device_owned_by_monitor {
            return Err(MonitorError::MonitorDoesNotOwnDevice);
        }
        if !policy.iommu_owned_by_monitor {
            return Err(MonitorError::IommuNotOwned);
        }
    }
    if !request.input_was_quarantined || !policy.quarantine_region_isolated {
        return Err(MonitorError::QuarantineBypass);
    }
    if !context.sanitized_view_immutable {
        return Err(MonitorError::MutableSanitizedView);
    }
    Ok(())
}
...
mod tests {
    ...
    fn policy() -> MonitorPolicy {
        MonitorPolicy {
            ...
            policy_epoch: 3,
            main_kernel_digest: [1; 32],
            direct_device_passthrough_denied: true,
            iommu_owned_by_monitor: true,
            quarantine_region_isolated: true,
        }
    }
    fn request() -> MainKernelRequest {
        MainKernelRequest {
            ...
            kernel_digest: [1; 32],
            subject_id: 2,
            resource: ResourceClass::Storage,
            capability_id: 3,
            input_digest: [2; 32],
            policy_epoch: 3,
            sequence: 5,
            deadline_monotonic_ns: 20,
            requests_direct_hardware: false,
            input_was_quarantined: true,
    ...
    fn context() -> MonitorContext {
        MonitorContext {
            now_monotonic_ns: 10,
            last_sequence: 4,
            capability_bound_to_subject: true,
            device_owned_by_monitor: true,
            sanitized_view_immutable: true,
        }
    }
    #[test]
    fn mediated_request_passes() {
        assert_eq!(admit(&policy(), &request(), &context()), Ok(()));
    }
    #[test]
    fn direct_hardware_denied() {
        let mut r = request();
        r.requests_direct_hardware = true;
        assert_eq!(
            admit(&policy(), &r, &context()),
            Err(MonitorError::DirectHardwareDenied)
        );
    }
    #[test]
    fn monitor_must_own_device() {
        let mut c = context();
        c.device_owned_by_monitor = false;
        assert_eq!(
            admit(&policy(), &request(), &c),
            Err(MonitorError::MonitorDoesNotOwnDevice)
        );
    }
    #[test]
    fn quarantine_cannot_be_bypassed() {
        let mut r = request();
        r.input_was_quarantined = false;
        assert_eq!(
            admit(&policy(), &r, &context()),
            Err(MonitorError::QuarantineBypass)
        );
    }
    #[test]
    fn replay_rejects() {
